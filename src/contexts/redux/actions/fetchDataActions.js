...
import {
  userRequestPDF,
  userRequestPDFFailed,
  userRequestPDFSuccesful,
} from "../../../reducers/userActionsReducers";
...
export const fetchTransfers = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const response = await API.get("users/banking/transfer_history/", config);
...
export const fetchOrders = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const transactionData = [].concat(
      ...response.data.map((item) => item.orders)
    );
...
export const fetchShares = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const sharesData = [].concat(
      ...response.data.map((item) => item.properties)
    );
...
export const fetchOrderForProperty =
...
export const fetchLinkedAccounts = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const response = await API.get("users/banking/linked/accounts/", config);
...
export const fetchNotificationsForUser =
  (page = 1, pageSize = 4, maxPageSize = 5) =>
...
export const fetchPublicPDFs =
  (pdfName, setSearchParams) => async (dispatch) => {
