...
import { fetchTransfers } from "./fetchDataActions";
...
export const initiatePlaid = (initiationType) => async (dispatch, getState) => {
  ...
  try {
    ...
    const response = await API.get("users/banking/info/", config);
...
export const createTransferIntent = () => async (dispatch, getState) => {
  ...
  try {
    const {
      ...
      plaid: {
        plaidInfo: { transferAmount, account },
      },
    } = getState();
...
export const exchangePublicTokenForAccessToken =
  (public_token) => async (dispatch, getState) => {
    dispatch(plaidAuthRequest());
    try {
      const {
        userAuth: { userInfo },
        plaid: {
          plaidInfo: { initiationType },
        },
      } = getState();
...
export const getLinkedAccounts = () => async (dispatch, getState) => {
  ...
  try {
    ...
    const response = await API.get("/users/banking/get_accounts/", config);
...
export const authorizeAndCreateTransfer = () => async (dispatch, getState) => {
  ...
  try {
    const {
      ...
      plaid: {
        plaidInfo: { transferAmount, account, type, description },
      },
    } = getState();
...
export const simulateTransferEvent =
  (eventData) => async (dispatch, getState) => {
    dispatch(
      plaidAuthRequest({
        loadingRequest: true,
      })
