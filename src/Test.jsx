function Test() {
  ...
  function formatTableToClipboardContent(dynamicLetters, fields_meta) {
    const headerRow = dynamicLetters.join("\t");
    const dataRows = fields_meta.map((field) => {
      const rowData = Object.keys(field).map((key) => String(field[key]));
      return rowData.join("\t");
    });
  ...
  function copyTableToClipboard(dynamicLetters, fields_meta) {
    const clipboardContent = formatTableToClipboardContent(
      dynamicLetters,
      fields_meta
    );
    ...
    // You can add a success message or other handling here if needed
  ...
  return (
    <main className="min-h-screen flex flex-1 items-center justify-center">
      {/* "Copy" button */}
      <button
        className="text-white p-1 sclale-90 hover:scale-105 hover:bg-gray-400/20 rounded-lg focus:scale-100 focus:bg-gray-400/20"
        onClick={() => copyTableToClipboard(dynamicLetters, fields_meta)}
      >
        <svg
          className="w-6 h-6"
