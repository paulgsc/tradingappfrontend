...
const websocketBaseUrl = import.meta.env.DEV
  ? "ws://127.0.0.1:8000"
  : "wss://tradingapp.up.railway.app";
...
export const fetchPropertiesQuery = (token) => {
  ...
  const fetchProperties = async () => {
    try {
      ...
      const response = await API.get("data/admin/property_list/", config);
      return response.data;
    } catch (error) {
      throw new Error("Error fetching data");
    }
  ...
  const fetchProperties = async () => {
    try {
      ...
      const response = await API.get(
        `data/admin/property_list/${propertyId}`,
        config
      );
      return response.data;
    } catch (error) {
